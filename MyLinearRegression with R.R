...
## Correlation test is used to evaluate the association between two (1 dependent vairable and 1 independant) variables.
...
## Trying  to see with what loan purpose the average of interest rate in training data
...
### for showing the ld_all$state data with unique values
...
### creating the dummies by using the function [line no = 259  ] for state and home.ownership
...
###### calculating the average by using f1 and f2
...
### the value of interest.rate recieved from train data set will be imposed(apply) on test data set
...
## The nrow R function returns the number of rows that are present in a data frame or matrix.The nrow R function returns the number of rows that are present in a data frame or matrix.
...
fit=lm(Interest.Rate ~ Monthly.Income + Revolving.CREDIT.Balance +
...
ld_train1[!rownames(ld_train1) %in% c(1621 , 1660 , 2117) , ]



########### now run the steps from line no 534
